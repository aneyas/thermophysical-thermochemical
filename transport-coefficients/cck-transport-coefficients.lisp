...
(defmethod mu1-1 ((model (eql 'cck))
		(coefficients hard-sphere-potential) temperature &key)
...
(defmethod lambda-ig1-1 ((model (eql 'cck))
		     (coefficients lennard-jones-6/12-potential) temperature
		     &key)
...
(define-test lambda1-1-cck
  ...
  (let ((lisp-unit:*epsilon* 5e-2))
    (let ((coeffs (make-species-lennard-jones-6/12-potential :Ar)))
      (assert-number-equal 12.5e-3 (lambda-ig1-1 'cck coeffs 200.0))
      (assert-number-equal 17.9e-3 (lambda-ig1-1 'cck coeffs 300.0))
      (assert-number-equal 21.7e-3 (lambda-ig1-1 'cck coeffs 380.0)))
...
(defmethod lambda1-1 ((model (eql 'cck))
		      (coefficients lennard-jones-6/12-potential) temperature cv
		      &key)
