;; Mirko Vukovic
;; Time-stamp: <2011-10-18 09:26:44 kt-transport-coeffs.lisp>
;; 
...
(defmethod mu-0 ((coefficients lennard-jones-6/12-potential) temperature)
...
"
  (with-slots (mass sigma epsilon/K) coefficients
    (* (/ 5 16)
       (/ (sqrt (* +pi+ mass (/ +amu+ 1e-20) (/ +k+ 1e-20) temperature))
	  (* +pi+ (expt sigma 2)
...
(defmethod mu-0 ((coefficients hard-sphere-potential) temperature)
  "Hard-sphere viscosity

Kee et al, (12.50)"
  (with-slots (mass sigma) coefficients
    (* (/ 5 16)
       (/ (sqrt (* +pi+ mass (/ +amu+ 1e-20) (/ +k+ 1e-20) temperature))
	  (* +pi+ (expt sigma 2))))))
...
(defmethod lambda-0 ((coefficients lennard-jones-6/12-potential) temperature)
...
(defmethod lambda-0 ((coefficients hard-sphere-potential) temperature)
  "Thermal conductivity using the hard-sphere model

Kee et al, 12.57"
  (with-slots (species sigma mass) coefficients
    (let ((Cv (Cv species temperature)))
    (* (/ 25d0
	  (* 32d0 (sqrt +pi+)))
       (sqrt (/ (* +k+ temperature)
